...
Для работы генерации ответов необходимо сначала скачать и запустить **Qwen Proxy** из другого репозитория. Убедитесь, что прокси-сервер активен, доступен по локальному адресу и не блокируется фаерволом. Без него модуль генерации не сможет подключиться к Qwen API.
